...
pt_t vector_avg(vector_t *vector) {
    pt_t avg = {0};
...
vector_t *plot_rect(double width, double height, double rad,
    ...
    pt_t hold[4] = {{0.0, 0.0}, {width, 0.0}, {width, height}, {0, height}};
...
bool intersect(vector_t *objA, vector_t *objB) {
    bool collided = false;
    for (int i = 0; i < objA->size; i++) {
        for (int j = 0; j < objB->size; j++) {
            double crosses1 = cross_products(i, j, objA, objB);
            double crosses2 = cross_products(j, i, objB, objA);
